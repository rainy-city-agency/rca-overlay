...
RainyDay.prototype.initialize = function(options) {
  ...
  this.imgDownscaled = this.customDrop || downscaleImage(this.img, options.reflectionWidth || 220)
  if (options.sound) {
  ...
  // Start rain engine
  this.rain(
    options.presets || [[3, 5, 2]],
    typeof options.speed === 'undefined' ? 50 : options.speed
  )
}
...
Drop.prototype.draw = function () {
  ...
  var rx = Math.max(0.8, this.r * 0.76)
  var ry = Math.max(1.2, this.r * (1.24 + Math.min(0.45, (this.yspeed || 0) * 0.18)))
  this.context.ellipse(this.x, this.y, rx, ry, 0, 0, Math.PI * 2, true)
  ...
  var shade = this.context.createRadialGradient(
    this.x - rx * 0.18,
    this.y - ry * 0.28,
    Math.max(1, rx * 0.12),
    this.x,
  ...
  shade.addColorStop(1, 'rgba(30, 55, 85, 0.32)')
  this.context.fillStyle = shade
  this.context.fillRect(this.x - rx, this.y - ry, rx * 2, ry * 2)

  var hx = this.x - rx * 0.32
  var hy = this.y - ry * 0.38
  var hr = Math.max(1.2, rx * 0.42)
  var highlight = this.context.createRadialGradient(hx, hy, 0, hx, hy, hr)
  highlight.addColorStop(0, this.r >= 4 ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.78)')
  highlight.addColorStop(0.35, 'rgba(255,255,255,0.42)')
  highlight.addColorStop(1, 'rgba(255,255,255,0)')
  this.context.fillStyle = highlight
  this.context.fillRect(this.x - rx, this.y - ry, rx * 2, ry * 2)

  this.context.beginPath()
  this.context.ellipse(this.x, this.y, rx, ry, 0, 0, Math.PI * 2)
  this.context.strokeStyle = 'rgba(28, 52, 82, 0.42)'
  this.context.lineWidth = this.r >= 6 ? 1.7 : 1.15
  this.context.stroke()
  this.context.beginPath()
